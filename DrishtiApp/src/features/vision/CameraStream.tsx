...
const CameraStream = () => {
  ...
  const {
    isConnected, connect, sendFrame, sendRaw,
    lastAlert, lastMessageType, lastMessagePriority, lastMessageFast,
  } = useStore();
  ...
  const enqueueAlert = useCallback((alert: string, priority: boolean, messageType: string, fast: boolean) => {
    const alertLower = alert.toLowerCase();
    const isCritical = CRITICAL_PREFIXES.some((p) => alertLower.startsWith(p)) ||
      (alertLower.includes("0.5 meters") && ["car", "motorcycle", "bus", "truck"].some((v) => alertLower.includes(v)));
    ...
    if (ttsQueue.current.length >= 4) ttsQueue.current = ttsQueue.current.slice(-3);
    ttsQueue.current.push({ text: alert, fast });

    if (!isSpeaking.current) speakNext();
  }, [speakNext]);

  // ✅ CLEANED: Wake Word simply delegates to useVoice
  const handleWakeWord = useCallback(() => {
    if (isListening || !isFocused) return;
    ttsQueue.current = []; // Clear Guardian backlog
    isSpeaking.current = false;
    startListening(); 
  ...
  useEffect(() => {
    ...
    enqueueAlert(lastAlert, lastMessagePriority ?? false, lastMessageType ?? "", lastMessageFast ?? false);
  }, [lastAlert, lastMessageType, lastMessagePriority, lastMessageFast, isListening, isFocused, enqueueAlert]);
  ...
  // ✅ CLEANED: Tap handler delegates to useVoice
  const handleTap = useCallback(() => {
    ...
    tapTimer.current = setTimeout(() => {
      ...
      if (taps >= 3) {
        ...
        Tts.stop();
        isSpeaking.current = false;
        Tts.speak("Emergency SOS activated.");
  ...
  if (!device) return <View style={styles.container}><Text style={styles.error}>Camera not found</Text></View>;

  return (
    <Pressable style={styles.container} onPress={handleTap}>
      <Camera ref={camera} style={StyleSheet.absoluteFill} device={device} isActive={isFocused} photo={true} />
      <View style={styles.overlay}>
        <Text style={styles.status}>{isConnected ? "🟢 Guardian Active" : "🔴 Reconnecting..."}</Text>
        {isListening ? (
          ...
          <View style={alertBoxStyle}>
            <Text style={[styles.alertText, isFastAlert && styles.alertTextUrgent, isClear && styles.alertTextClear]}>
              {lastAlert || "Scanning..."}
            </Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  overlay: { position: "absolute", bottom: 40, left: 16, right: 16, alignItems: "center", gap: 10 },
  status: { color: "white", fontSize: 12, backgroundColor: "rgba(0,0,0,0.7)", paddingHorizontal: 15, paddingVertical: 4, borderRadius: 20 },
  alertBox: { padding: 20, backgroundColor: "rgba(0,0,0,0.85)", borderRadius: 15, width: "100%", borderWidth: 2, borderColor: "#FFD700" },
  alertBoxUrgent: { borderColor: "#FF3B30", backgroundColor: "rgba(255,59,48,0.25)" },
  alertBoxClear: { borderColor: "#34C759", backgroundColor: "rgba(52,199,89,0.15)" },
  alertBoxInfo: { borderColor: "#34C759", backgroundColor: "rgba(52,199,89,0.1)" },
  alertBoxRead: { borderColor: "#007AFF", backgroundColor: "rgba(0,122,255,0.1)" },
  alertText: { color: "#FFD700", fontSize: 20, textAlign: "center", fontWeight: "bold" },
  alertTextUrgent: { color: "#FF3B30", fontSize: 22 },
  alertTextClear: { color: "#34C759" },
  listeningBox: { padding: 24, backgroundColor: "rgba(0,122,255,0.9)", borderRadius: 15, width: "100%", borderWidth: 2, borderColor: "#fff", alignItems: "center" },
